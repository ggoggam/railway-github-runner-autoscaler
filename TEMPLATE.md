...
Paste the following as the template overview. The structure follows Railway's
[best practices](https://docs.railway.com/templates/best-practices#overview).

---

# Deploy and Host Self-Hosted GitHub Actions Runners with Railway

A pool of ephemeral self-hosted GitHub Actions runners that scales itself. An
autoscaler service listens for `workflow_job` webhooks, tracks how many jobs are
waiting or running, and drives the runner pool's replica count to match —
growing the moment work arrives and shrinking only once it is safe to. Each
runner takes exactly one job and exits, so nothing leaks between jobs.

## About Hosting Self-Hosted GitHub Actions Runners

Self-hosted runners are usually run either always-on, which you pay for around
the clock, or hand-scaled, which means someone notices the queue. Automating it
is harder than it looks: the platform picks which replica to terminate and
cannot know which one is mid-job, GitHub never retries a failed webhook, and a
runner that exits without its container being rebuilt leaves a replica that
serves nothing. This template handles all three — scale-down is gated on
in-progress work, an idle cooldown, and a startup grace; and the autoscaler
reconciles against the GitHub API each cycle so a lost webhook or a dead pool
recovers on its own.

## Common Use Cases

- Cutting CI spend by scaling runners to zero between jobs instead of paying for idle capacity
- Running jobs that need access to a private network, internal registry, or database
...
## Dependencies for Self-Hosted GitHub Actions Runner Hosting

- A GitHub repository with Actions enabled
- A GitHub personal access token, for runner registration and job reconciliation
- A Railway workspace token, so the autoscaler can change the runner pool's replica count
- `myoung34/github-runner`, which provides the runner container

### Deployment Dependencies

- [GitHub Actions self-hosted runners](https://docs.github.com/en/actions/hosting-your-own-runners)
- [myoung34/github-runner](https://github.com/myoung34/docker-github-actions-runner)
- [Railway API tokens](https://railway.com/account/tokens)
- [Autoscaler source and configuration reference](https://github.com/ggoggam/railway-github-runner-autoscaler)

### Implementation Details

After deploying, three steps in GitHub finish the setup.

Add a webhook under **Settings → Webhooks → Add webhook** on the repository:

- Payload URL: `https://<your-autoscaler-domain>/webhook`
- Content type: `application/json`
- Secret: the `GITHUB_WEBHOOK_SECRET` value from the Autoscaler service
- Events: *Let me select individual events* → **Workflow jobs** only

Then point a job at the pool:
...
The labels must match `RUNNER_LABELS`. Check `GET /status` on the autoscaler for
current queued and in-progress counts and the live runner count.

Run exactly one replica of the Autoscaler. Job state is held in memory, so a
second replica would scale against its own partial view.

### Why Deploy Self-Hosted GitHub Actions Runners on Railway?
...
Horizontal scaling is the whole mechanism here: the autoscaler does its work by
setting a replica count through Railway's API, so the pool is ordinary Railway
infrastructure that happens to be driven by your CI queue. By deploying
self-hosted runners on Railway, you are one step closer to supporting a complete
